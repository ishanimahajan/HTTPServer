...
}}(function(){var n=$(".emailfromquery");
var o;
var c;
var e=$("#errorpages");
var a=$("#formid").data("mode");
var l=window.location.search.substring(1);
if(n.length>0){var m={submitAction:function(){this.toggleSubmitButton("disabled");
var p="",q=(""===trials.apikey)?trials.endPoint:trials.endPoint+"?api_key="+trials.apikey;
if(typeof window.adobeIMS!=="undefined"&&window.adobeIMS.getAccessToken()!==null){p=m.queryStringToJSON()
}else{p=m.queryStringToJSON()
}if(this.hasTokenParam()){if(this.getToken()!=undefined||this.getToken()!=null){p.token=this.getToken()
}else{window.location.href="/404.html"
}}trials.postCommonService("",p,q)
},postSubmitSuccess:function(r,t,q){var p=e.attr("data-thankyoupage")+".html";
if(this.getToken()){if(!r.successful){p="/404.html"
}}window.location.href=p
},postSubmitFailure:function(p,r,q){if(p.status==502||p.status==503){window.location.href=e.attr("data-serviceunavailableerrorpage")+".html"
}else{if(p.status!=200){window.location.href=e.attr("data-genericerrorpage")+".html"
}}},postSubmitAlways:function(){},queryStringToJSON:function(){var r=location.search.slice(1).split("&");
var p={};
var t={};
var q=$('input[type="checkbox"]');
r.forEach(function(w,u){w=w.split("=");
t[w[0]];
for(var v=0;
v<o.length;
v++){if(o[v][w[0]]){p[w[0]]=decodeURIComponent(w[1]||"")
}}});
Array.prototype.forEach.call(q,function(v,u){if(q[u].checked==true){p[q[u].value]="true"
}});
if(f){p.sid=c
}p.imstoken=(typeof window.adobeIMS!=="undefined"&&typeof window.adobeIMS._profile!=="undefined"&&window.adobeIMS._profile!==null)?window.adobeIMS.getAccessToken():null;
p.clientid=(typeof window.adobeIMS!=="undefined"&&typeof window.adobeIMS._profile!=="undefined"&&window.adobeIMS._profile!==null)?window.adobeIMS.getClientID():null;
return JSON.parse(JSON.stringify(p))
},urlParam:function(p){var r=l.split("&"),q,t;
for(q=0;
q<r.length;
q++){t=r[q].split(/=(.+)/);
if(t[0]==p){return decodeURIComponent(t[1]).replace(/[<>]/g,"")
}}},getToken:function(){return this.urlParam("token")
},getUrlVars:function(){var u=[],t,p,r=window.location.href,q;
if(r.lastIndexOf("#")!=-1){r=r.substr(r,r.lastIndexOf("#"))
}p=r.slice(r.indexOf("?")+1).split("&");
for(q=0;
q<p.length;
q++){t=p[q].split("=");
u.push(t[0]);
...
},hasTokenParam:function(){return l.indexOf("token=")!=-1
},toggleSubmitButton:function(r){var q={};
var t="style";
var p="disabled";
if(r){q={"background-color":"grey",cursor:"default"};
t="";
p=""
}$(".actionButton").attr("disabled","disabled").css(q).removeAttr(t).removeAttr(p)
}};
if(typeof window.actionRegistry!=="undefined"){actionRegistry.register("form.unsubscribe.action",m)
}var g=m.getUrlVars()["email"],h=m.getUrlVars()["type"],f=m.getUrlVars()["sname"],d="<strong>"+decodeURIComponent(g).replace(/</g,"").replace(/>/g,"")+"</strong>";
$(".grid-cols-10.grid-offset-3.grid-padded-v6x").find("a:first").on("click",function(){this.href=this.href+window.location.search
});
var k=function(q){var r=$(q).find("[type=checkbox]");
for(var p=0;
p<r.length;
p++){if(r[p].checked){return false
}}return true
};
var j=function(p){if(p.target.checked){m.toggleSubmitButton();
return
}if(k(p.target.parentElement.parentElement)){m.toggleSubmitButton(true)
}};
$.getJSON(window.location.href.replace(".html",".unsubscribetypes.json"),function(w){$(".unsubscribe-hidden").removeClass("unsubscribe-hidden");
var p=w.types[h+(f?"-"+f:"")];
if(typeof(a)=="undefined"&&!p){window.location.href="/404.html"
}n.html(w.types[h+(f?"-"+f:"")].headerText.replace("####",d));
var q=p.textvalueCheckboxes;
q.forEach(function(D,A){var z=D.split("|||");
var C=z[0];
var y=z[1];
var B="";
if(A==0){B='checked="checked"'
}$("#chechBoxContainer").append('<label for="false" class="text-light"><input class="unsubscribe-check" type="checkbox" name="unsubscribe" value="'+y+'" '+B+"/> <span>"+C+"</span></label><br />")
});
$(".unsubscribe-check").each(function(y,z){z.onchange=j
});
var u={names:q.toString().replace(/,$/,"").split(",").map(function(y){return{CheckboxText:y}
})};
c=w.types[h+(f?"-"+f:"")].subTypeValue;
o=w.params;
regexEmail=/\S+@\S+\.\S+/;
for(var t in o){var x=o[t];
var v=Object.keys(o[t])[0];
var r=Object.keys(o[t])[1];
if(typeof(a)=="undefined"){if(x[v]=="true"){if(r){if((!m.getUrlVars()[v]||m.getUrlVars()[v].toLowerCase()=="null"||!m.getUrlVars()[r]||m.getUrlVars()[r].toLowerCase()=="null")&&x[r]==m.getUrlVars()[r]||!regexEmail.test(d)){window.location.href="/404.html"
}}else{if(!m.getUrlVars()[v]||m.getUrlVars()[v].toLowerCase()=="null"||!regexEmail.test(d)){window.location.href="/404.html"
}}}}}})
}})();
